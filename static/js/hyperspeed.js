...
// ---- Distortion presets (unchanged math from the original component) ----
...
class App {
  constructor(container, options = {}) {
    ...
    this.speedUpTarget = options.baseSpeed ?? 0.15;
    this.speedUp = options.baseSpeed ?? 0.15;
    this.timeOffset = 0;
  ...
  initPasses() {
    ...
    this.bloomPass = new EffectPass(this.camera, new BloomEffect({
      ...
      intensity: 0.8,
      mipmapBlur: true,
      resolutionScale: options.lowPower ? 0.5 : 1
    }));
    this.renderPass.renderToScreen = false;
    this.bloomPass.renderToScreen = true; // overridden to false below only once SMAA takes over
    this.composer.addPass(this.renderPass);
    this.composer.addPass(this.bloomPass);

    if (!options.lowPower) {
      this.bloomPass.renderToScreen = false; // SMAA becomes the final pass instead
  ...
  resume() {
    ...
    this.paused = false;
    this.clock.getDelta(); // drop the idle gap so the scene doesn't jump
    this.tick();
  }
  ...
  tick() {
    ...
    const delta = Math.min(this.clock.getDelta(), 0.1); // clamp so a stall doesn't jump the scene
    this.render(delta);
    this.update(delta);
    this.rafId = requestAnimationFrame(this.tick);
  }
...
class CarLights {
  ...
  init() {
    ...
    let geometry = new THREE.TubeGeometry(curve, 40, 1, 16, false);
...
const roadMarkings_fragment = `
  ...
  color = mix(color, uShoulderLinesColor, brokenLines);
`;
...
function resizeRendererToDisplaySize(renderer, setSize) {
  ...
  // canvas.width/height are the internal drawing-buffer size, which THREE
  // scales by pixel ratio. Comparing that against the raw CSS clientWidth
  // (no pixel ratio applied) mismatches on every HiDPI screen and forces a
  // full resize + render-target reallocation on every single frame — that
  // constant thrash is what caused the stutter. Compare like-for-like instead.
  const pixelRatio = renderer.getPixelRatio();
  const expectedWidth = Math.round(clientWidth * pixelRatio);
  const expectedHeight = Math.round(clientHeight * pixelRatio);
  const needResize = canvas.width !== expectedWidth || canvas.height !== expectedHeight;
  if (needResize) setSize(clientWidth, clientHeight, false);
  return needResize;
}

/**
 * Initialize the Hyperspeed background inside `container`.
 * Returns the App instance — call `.dispose()` to tear it down.
 */
export function initHyperspeed(container, effectOptions = {}) {
  const DEFAULT = {
    ...
    colors: {
      ...
      leftCars: [0x22d3ee, 0x6366f1, 0x38bdf8],
      rightCars: [0x6366f1, 0x22d3ee, 0x818cf8],
      sticks: 0x22d3ee
    }
  };

  const options = { ...DEFAULT, ...effectOptions, colors: { ...DEFAULT.colors, ...(effectOptions.colors || {}) } };
